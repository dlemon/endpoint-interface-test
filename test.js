...
function run_test(max = 5000) {
    let success = 0;
    let count = 0;
    let errors = {};

    console.log('posting', max, 'simulteanuous requests to add a vehicle via', url);
    let start = Date.now();
...
//log network address we are posting from
  ...
  Object.keys(ifaces).forEach(dev => {
    ifaces[dev].filter(details => {
      if (details.family === 'IPv4' && details.internal === false) {
        ...
        console.log('external IP address of client: ',address);
      }
    });
  });
...
//log the endpoint address and then start the test.
http.get('http://equiptrace.nl:8080/', function(res) {
    console.log('IP Address equiptrace.nl     : ', res.connection.remoteAddress);
    console.log();

    // Parse commandline and run the requested test
    // Wait until we are connected, else the output is out of order
    // because of parallel execution of the get and the test itself.
    if (3 == process.argv.length) {
        run_test(Number(process.argv[2]));
    } else {
        run_test()
    }
});
